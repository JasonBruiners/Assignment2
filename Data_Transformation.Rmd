...
timesLIVE_likes <- separate(TimesLIVE_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% summarise(timesLIVE = sum(favorite_count))

Ewnupdate_likes <- separate(News24_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% summarise(Ewnupdate = sum(favorite_count))

IOL_likes <- separate(IOL_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% summarise(IOL = sum(favorite_count))

eNCA_likes <- separate(ewnupdates_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% summarise(eNCA = sum(favorite_count)) 

SABC_likes <- separate(SABCNews_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% summarise(SABC = sum(favorite_count))
