...
# Addtional cleaning per dataframe
...
tweets.News24_stem <- tweets.News24_clean %>% select(stripped_text) %>% unnest_tokens(word, stripped_text)
...
News24_cleaned$stripped_text <- gsub("http.+ |http.+$", " ", News24_cleaned$stripped_text) # remove links
...
News24_tester <- News24_cleaned$stripped_text
News24_tester
```
...
```{r}
testywesty <- tweets.News24_clean %>% select(stripped_text)
testywesty
```
...
corpusNews24 <- Corpus(VectorSource(testywesty))
...
top10terms_5 = as.matrix(terms(News24_lda_5,10))
top10terms_2 = as.matrix(terms(News24_lda_2,10))
top10terms_10 = as.matrix(terms(News24_lda_10,10))
