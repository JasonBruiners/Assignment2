...
News24_tweets <- separate(News24_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% mutate(date = as.Date(date))
timesLIVE_tweets <- separate(TimesLIVE_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% mutate(date = as.Date(date))
Ewnupdate_tweets <- separate(ewnupdates_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% mutate(date = as.Date(date))
IOL_tweets <- separate(IOL_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% mutate(date = as.Date(date))
eNCA_tweets <- separate(eNCA_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% mutate(date = as.Date(date))
SABC_tweets <- separate(SABCNews_tweets, created_at, into=c("date","time"), sep=" ") %>% group_by(date) %>% mutate(date = as.Date(date))
```


```{r}
# Sentiment Analysis
# this example uses library(syuzhet)
...
# this example uses library(tidytext) and library(textdata)
## AFFIN sentiment analysis

#AFFIN_tweets <- cleaned_tweets %>% 
