...
myfuncEwnupdate <- function(x,y){Ewnupdate_tweets[Ewnupdate_tweets$date >= x & Ewnupdate_tweets$date <= y,]}
...
myfuncSABC <- function(x,y){SABC_tweets[SABC_tweets$date >= x & SABC_tweets$date <= y,]}
...
Ewnupdate_tweets <- myfuncEwnupdate(DATE1,DATE2)
IOL_tweets <- myfuncIOL(DATE1,DATE2)
eNCA_tweets <- myfunceNCA(DATE1,DATE2)
SABC_tweets <- myfuncSABC(DATE1,DATE2)
...
```{r}
News24_STM <- stm(documents = out$documents, vocab = out$vocab,
              K = 10, max.em.its = 75, data = out$meta,
              init.type = "Spectral", verbose = FALSE)
```
...
timesLIVE_freq <- timesLIVE_tweets %>% group_by(date) %>% summarise(timesLIVE = n())
Ewnupdate_freq <- Ewnupdate_tweets %>% group_by(date) %>% summarise(Ewnupdate = n())
IOL_freq <- IOL_tweets %>% group_by(date) %>% summarise(IOL = n())
eNCA_freq <- eNCA_tweets %>% group_by(date) %>% summarise(eNCA = n()) 
SABC_freq <- SABC_tweets %>% group_by(date) %>% summarise(SABC = n())
